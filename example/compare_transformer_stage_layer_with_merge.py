...
def _write_html(path: Path, cand: Candidate, num_pes: int) -> None:
    res = cand.result
    write_schedule_html(
        path,
        title=f"transformer {cand.mode} schedule",
        meta={
            "network": "transformer",
            "mode": cand.mode,
        ...
        state_batches=[int(x) for x in res.milp_solution.state_batches],
        state_active_blocks=[[int(v) for v in row] for row in res.state_active_blocks],
...
def main() -> None:
    ...
    stage_html = out_dir / "best_stage_detail.html"
    layer_html = out_dir / "best_layer_detail.html"
    ...
    _write_html(stage_html, best_stage, args.num_pes)
    _write_html(layer_html, best_layer, args.num_pes)
