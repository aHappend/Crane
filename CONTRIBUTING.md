...
- Matched-evaluator calibration, additional profile/network coverage and
  extensions beyond the current uniform-cohort training reference.
...
The recorded experimental results are regenerated by `experiments.report`, not
edited to make a comparison favorable. It verifies raw result and workload hashes.
When adding an expected infeasible case, state the policy and capacity argument;
do not classify timeouts or unexpected solver failures as successful outcomes.

Core profiles are measured with pinned upstream source and checked by checksum.
Do not combine a native SET total-cost figure with the Python outer evaluator
and label the ratio a reproduced paper speedup without calibration evidence.
